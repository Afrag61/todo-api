...
type WithParentParams<T extends string> = RouteParameters<":todoId"> &
  RouteParameters<T>;
...
router.post<typeof route, WithParentParams<typeof route>>(
  "/add-sub-todo",
  async (req, res) => {
...
router.get<typeof route, WithParentParams<typeof route>>(
  "/:subTodoId",
  async (req, res) => {
    ...
    try {
      ...
      if (subTodo) {
        ...
        res
          .status(404)
          .json({ isSuccess: false, error: "Cannot find the sub todo." });
      }
    } catch (error) {
      res
        .status(500)
        .json({ isSuccess: false, error: "Error getting sub todo." });
    }
  }
);

router.patch<typeof route, WithParentParams<typeof route>>(
  "/:subTodoId",
  async (req, res) => {
    const { todoId, subTodoId } = req.params;
...
router.delete<typeof route, WithParentParams<typeof route>>(
  "/:subTodoId",
  async (req, res) => {
    const { todoId, subTodoId } = req.params;
    ...
    try {
      ...
      res
        .status(500)
        .json({ isSuccess: false, error: "Error deleting sub todo." });
    }
  }
);
...
router.patch<typeof route, WithParentParams<typeof route>>(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ isSuccess: false, error: "Error checking sub todo." });
    }
  }
);
